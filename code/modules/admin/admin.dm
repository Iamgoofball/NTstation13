...
/datum/admins/proc/Secrets()
	...
	if(check_rights(R_FUN,0))//TODO: Make all this hardcoded random event panel stuff its own proc so we can actually remove poor events without making the code stop compiling
		dat += {"
			...
			<A href='?src=\ref[src];secretsfun=npcblob'>Spawn an NPC blob</A><BR>
			<A href='?src=\ref[src];secretsfun=goblob'>Spawn a blob overmind</A><BR>
			<A href='?src=\ref[src];secretsfun=aliens'>Trigger an Alien infestation</A><BR>
			<A href='?src=\ref[src];secretsfun=alien_silent'>Spawn an Alien silently</A><BR>
			<A href='?src=\ref[src];secretsfun=brand'>Trigger a brand intelligence event</A><BR>
